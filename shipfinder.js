...
const shipList = {
  ...
  "HORIZON SPIRIT": {
    ...
    geometry: {
      type: "Point",
      coordinates: [-122.337023, 37.800348],
    },
  },
  "HYUNDAI MARS": {
    properties: {
    ...
    geometry: {
      type: "Point",
      coordinates: [-122.320003, 37.813302],
    },
  },
  "ONE HANGZHOU BAY": {
    properties: {
    ...
    geometry: {
      type: "Point",
      coordinates: [-122.328717, 37.810767],
    },
  },
  "EVER LOYAL": {
    properties: {
    ...
    geometry: {
      type: "Point",
      coordinates: [-122.338872, 37.80807],
    },
  },
  "MSC CAMILLE": {
    properties: {
    ...
    geometry: {
      type: "Point",
      coordinates: [-122.314923, 37.795898],
    },
  },
  "CAP SAN JUAN": {
    properties: {
  ...
  "NYK CONSTELLATION": {
    ...
    geometry: {
      type: "Point",
      coordinates: [-122.322775, 37.798065],
    },
  },
  "Hello\nThere": {
    properties: {
...
AFRAME.registerComponent("shipfinder", {
  init: function () {
    ...
    const shipToggle = document.querySelector("#ships-toggle");
    const shipmentsToggle = document.querySelector("#shipments-toggle");

    shipToggle.onclick = this.toggleData.bind(this);
    shipmentsToggle.onclick = this.toggleData.bind(this);
  ...
  selectShip(event) {
    ...
    ships.forEach((ship) => {
      ship.object3D.visible =
        ship.getAttribute("id") === selectedShip || !selectedShip; // if no ship selected, show all
    });
  },
  handleScale(event) {
  ...
  handleScroll(event) {
    ...
    ships.forEach((ship) => {
      ...
      ship.object3D.position.y = Math.min(Math.max(MINY, newPosition), MAXY);
    });
  },
  toggleData(event) {
